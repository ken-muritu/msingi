...
const priceRanges = [
  ...
  { label: 'Over KES 200,000', min: 200000, max: Infinity },
]
          ...
          <div className="sticky top-24 space-y-6">
            ...
            <div>
              ...
              <ul className="space-y-1">
                <li>
                  <a
                    href="/products"
                    className={`block text-sm px-2 py-1.5 rounded-lg transition-colors ${!categoryParam ? 'text-brand-700 bg-brand-50 font-medium' : 'text-slate-600 hover:bg-slate-50'}`}
                  >
                    All Products
                  </a>
                </li>
                {categories.map((cat) => (
                  <li key={cat.id}>
                    <a
                      ...
                      className={`block text-sm px-2 py-1.5 rounded-lg transition-colors ${categoryParam === cat.slug ? 'text-brand-700 bg-brand-50 font-medium' : 'text-slate-600 hover:bg-slate-50'}`}
                    >
                      {cat.name}
                    </a>
                  </li>
            ...
            {hasFilters && (
              <button
                ...
                className="w-full text-xs text-flash-DEFAULT hover:text-red-700 font-medium py-2 border border-red-200 rounded-lg transition-colors"
              >
                Clear Filters
              </button>
            )}
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center justify-between mb-4 gap-3">
            <button
              ...
              className="lg:hidden flex items-center gap-1.5 border border-slate-200 text-slate-700 px-3 py-2 rounded-xl text-sm hover:bg-slate-50 transition-colors"
            >
            ...
            <div className="flex items-center gap-2 ml-auto">
              ...
              <div className="relative">
                <select
                  ...
                  className="appearance-none border border-slate-200 rounded-xl px-3 py-2 pr-8 text-sm text-slate-700 bg-white focus:outline-none focus:ring-2 focus:ring-brand-500 cursor-pointer"
                >
        ...
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <div className="absolute right-0 top-0 h-full w-72 bg-white overflow-y-auto p-5">
            ...
            <div className="mb-5">
              ...
              {brands.map((brand) => (
                <label key={brand} className="flex items-center gap-2 text-sm py-1.5 cursor-pointer">
                  <input type="checkbox" checked={selectedBrands.includes(brand)} onChange={() => toggleBrand(brand)} className="accent-brand-600" />
                  {brand}
                </label>
              ))}
            </div>
            ...
            <div className="mb-5">
              ...
              {priceRanges.map((range) => (
                <label key={range.label} className="flex items-center gap-2 text-sm py-1.5 cursor-pointer">
                  <input type="radio" name="mobile-price" checked={priceRange?.min === range.min} onChange={() => setPriceRange({ min: range.min, max: range.max })} className="accent-brand-600" />
                  {range.label}
                </label>
              ))}
            </div>
            <button onClick={() => { clearFilters(); setFiltersOpen(false) }} className="w-full bg-slate-100 text-slate-700 py-2.5 rounded-xl text-sm font-medium mb-2">Clear Filters</button>
            <button onClick={() => setFiltersOpen(false)} className="w-full bg-brand-600 text-white py-2.5 rounded-xl text-sm font-semibold">Apply</button>
          </div>
        </div>
      )}
    </div>
